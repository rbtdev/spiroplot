...
var SpiroPlotter = {
	...
	initialize: function(armSpecs, startAngle, maxPenWidth, minPenWidth, zoomLevel) {
		...
		this.armSet = new ArmSet(armSpecs, {x:0,y:0}, 0, this.gMoving, this.toLocal.bind(this));
		this.refresh();		
	},
	...
	canvasDoubleClick: function (event) {
		...
		//this.stop();
		var mouseClick = {x: event.offsetX, y:event.offsetY};
	...
	computeSpiro: function (armSet) {
		...
		for (var t = 0; t<=maxTime+this.deltaT; t+=this.deltaT) {
			...
			points[i] = {point:point, t:t, speed:speed };
			i++;
		}
	...
	spiroPoint: function (t,armSet) {
		...
		// return the point
		return ({x:x,y:y, dx:dx, dy:dy, angle: angle});
	},
	...
	getColor: function (time, maxTime, colors) {
		...
		return ({r:colors[colorIndex].r, g: colors[colorIndex].g, b: colors[colorIndex].b});
	},
